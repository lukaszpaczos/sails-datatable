...
module.exports = (function () {
    ...
    var adapter = {
        ...
        datatable: function (connection, collection, options, cb) {
            ...
            if (options.groupBy && options.sumBy) {
                this.find().groupBy(options.groupBy).sum(options.sumBy).exec(function (err, data) {
            ...
            if(options.populate && !options.groupBy) {
            ...
            if (options.groupBy && options.sumBy) {
                findQuery.groupBy(options.groupBy).sum(options.sumBy);
            }
